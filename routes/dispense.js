...
router.post('/records', async (req, res) => {
    ...
    try {
        ...
        await pool.query(
            ...
            [rq_id, pt_name, rp_name, rp_relationship, pt_gender, pt_age, pt_contact, pt_address, item_name, brand, quantity]
        );
        ...
        const existingMedicine = await pool.query("SELECT * FROM inventory WHERE item_name = $1 AND brand = $2", [item_name, brand]);
        if (existingMedicine.rows.length > 0) {
            ...
            await pool.query("UPDATE inventory SET quantity = $1 WHERE item_name = $2 AND brand = $3", [newQuantity, item_name, brand]);
        } else {
